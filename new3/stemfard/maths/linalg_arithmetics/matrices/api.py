...
from stemfard.maths.linalg_arithmetics.matrices._docstring import (
    COMMON_DOCSTRING_PARAMS, inject_common_params
)
from stemfard.core.models import BuildSteps
...
def linalg_matrix_arithmetics(
    ...
    >>> A = np.array([[1, 2], [3, 4]])
    >>> B = np.array([[5, 6], [7, 8]])
    >>> steps = linalg_matrix_arithmetics(A, B, operations=("add", "multiply"))
    ...
    for i, op in enumerate(parsed.config.operations, 1):
        api = compute_map[op]
