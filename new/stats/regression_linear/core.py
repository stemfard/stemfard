...
class LinearRegressionCalculations(BaseLinearRegression):
    ...
    def html_str(self) -> HtmlStrings:
        ...
        if self.stderrors:
            ...
            stderrors = (
                f". Set \\( {str_color_values('stderrors=True')} \\) "
    ...
    def table_normal_coefs_latex(self) -> str:
        ...
        column_sums = (
            f"\\sum x_{{i}} = {self.total_x_rnd}",
            f"\\sum y_{{i}} = {self.total_y}",
            f"\\sum x_{{i}}y_{{i}} = {self.total_xy_rnd}",
            f"\\sum x_{{i}}^{{2}} = {self.total_x_squared_rnd}"
        )
    ...
    @cached_property
    def table_normal_stderrors_latex(self) -> str:
        ...
        column_sums = (
            f"\\sum x_{{i}} = {self.total_x_rnd}", 
            f"\\sum y_{{i}} = {self.total_y_rnd}", 
            f"\\sum x_{{i}}^{{2}} = {self.total_x_squared_rnd}", 
            f"\\sum \\hat{{y_{{i}}}} = {self.total_fitted_yhat_rnd}", 
            (
                f"\\sum \\left(y_{{i}} - \\hat{{y_{{i}}}}\\right)^{{2}} "
                f"= {self.total_residuals_squared_rnd}"
            )
        )
    ...
    def predict_latex(self) -> str:
        ...
        self.bse_counter += 1 # must update `self.counter` even though not used
        
        predict_xy_arr = self.calc_predict_at_x_rnd
        nrows = predict_xy_arr.shape[0]
        
        xis = [f"x_{{{idx}}}" for idx in range(1, self.bse_k)]
        xis_str = (f"\\( {', '.join(xis)} \\) and \\( x_{{{self.bse_k}}} \\)")
        fvars = (
            f"variable \\( x_{{1}} \\)"
            if self.bse_k == 2 else
            f"variables \\( {xis_str} \\)"
        )
        ...
        for idx in range(nrows):
            ...
            xis = [f"x{idx}" for idx in range(1, self.bse_k)]
            for xi in xis:
    ...
    def table_anova(self) -> list[str]:
        ...
        arr = [
            ...
            [self.sse, self.bse_n - self.bse_k, self.mse, nan, nan, nan],
            [self.sst, self.bse_n - 1, self.mst, nan, nan, nan]
        ]
        ...
        arr_latex = highlight_array_vals_arr(
            arr=around(arr, self.prm_decimals),
            brackets=None,
            index=index,
            col_names=col_names
        )
    ...
    def normal_coeffs_formula_expanded_latex(self) -> list[str]:
        ...
        steps_mathjax.append(
            ...
            f"\\( ({{{bse_counter}}}.1) \\) below."
        )
        steps_mathjax.append(
            f"\\( \\displaystyle\\quad\\beta_{{1}} "
            f"= \\frac{{\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: "
            f"x_{{i}} \\: y_{{i}} - n \\: \\bar{{x}} \\: "
            f"\\bar{{y}}}}{{\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: "
            f"x_{{i}}^{{2}} - n \\: \\bar{{x}}^{{2}}}} "
            f"\\qquad \\cdots \\qquad ({{{bse_counter}}}.1) \\)"
        )
        steps_mathjax.append(
            "Create the following table for ease of calculations of the "
            f"parameters that appear in Equation \\(({{{bse_counter}}}.1)\\) "
            "above."
        )
        ...
        steps_mathjax.append(
            f"\\( \\quad\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: x_{{i}} \\: "
            f"y_{{i}} = {self.total_xy_rnd} \\:, \\quad "
            f"\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: x_{{i}}^{{2}} "
            f"= {self.total_x_squared_rnd} \\)"
        )
        steps_mathjax.append(
            f"\\( \\displaystyle\\quad"
            f"\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: x_{{i}} \\quad"
            f"\\longrightarrow\\quad\\bar{{x}} = {self.total_x_rnd} "
            f"= \\frac{{1}}{{{self.bse_n}}} "
            f"\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: x_{{i}} "
            f"= \\frac{{{self.total_x_rnd}}}{{{self.bse_n}}} "
            f"= {self.mean_x_rnd} \\)"
        )
        steps_mathjax.append(
            f"\\( \\displaystyle\\quad"
            f"\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: y_{{i}} \\quad"
            f"\\longrightarrow\\quad\\bar{{y}} = {self.total_y_rnd} "
            f"= \\frac{{1}}{{{self.bse_n}}} "
            f"\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: y_{{i}}"
            f"= \\frac{{{self.total_y_rnd}}}{{{self.bse_n}}} "
            f"= {self.mean_y_rnd} \\)"
        )
        ...
        steps_mathjax.append(
            f"Substitute the above values into Equation \\(({bse_counter}.1)\\) "
        ...
        steps_mathjax.append(
            f"\\( \\quad = \\displaystyle\\frac{{{self.total_xy_rnd} "
            f"- {self.bse_n} \\left({self.mean_x_rnd}\\right) "
            f"\\left({self.mean_y_rnd}\\right)}}{{{self.total_x_squared_rnd} "
            f"- {self.bse_n} \\: \\left({self.mean_x_rnd}\\right)^{{2}}}} \\)"
        )
        
        n_mean_x_mean_y = self.bse_n * self.mean_x * self.mean_y
        n_mean_x_mean_y_rnd = around(n_mean_x_mean_y, self.prm_decimals)
        n_mean_x_squared = self.bse_n * self.mean_x ** 2
        n_mean_x_squared_rnd = around(n_mean_x_squared, self.prm_decimals)
        
        steps_mathjax.append(
            f"\\( \\quad = \\displaystyle\\frac{{{self.total_xy_rnd} "
            f"- {n_mean_x_mean_y_rnd}}}{{{self.total_x_squared_rnd} "
            f"- {n_mean_x_squared_rnd}}} \\)"
        )
        
        _numer = around(self.total_xy - n_mean_x_mean_y, self.prm_decimals)
        
        steps_mathjax.append(
            f"\\( \\quad "
            f"= \\displaystyle\\frac{{{_numer}}}{{{self.sb_denom_rnd}}} \\)"
        )
        steps_mathjax.append(f"\\(\\quad = {self.b1_normal_rnd} \\)")
        
        # ---------------------------
        #   b0 - Intercept
        # ---------------------------
        
        title = f"\\( {bse_counter}.2) \\: \\beta_{{0}} - \\text{{intercept}} \\)"
        steps_mathjax.append(html_bg_level2(title=title))
        
        steps_mathjax.append(
            "The regression coefficient \\( \\beta_{{0}} \\) "
            f"(i.e. intercept) is given by Equation \\(({bse_counter}.2) \\) "
            "below."
        )
        steps_mathjax.append(
            f"\\( \\quad\\beta_{{0}} = \\bar{{y}} "
            f"- \\beta_{{1}} \\: \\bar{{x}} "
            f"\\qquad \\cdots \\qquad ({bse_counter}.2) \\)"
        )
        
        steps_mathjax.append("where,")
        steps_mathjax.append(
            f"\\(\\quad\\bar{{y}} = {self.mean_y_rnd}\\:,"
            f"\\quad\\beta_{{1}} = {self.b1_normal_rnd}\\:,"
        ...
        steps_mathjax.append(
            ...
            f"\\( ({{{bse_counter}}}.2) \\) and work through to get the value of "
        ...
        steps_mathjax.append(f"\\( \\quad = {self.b0_normal_rnd} \\)")
        
        # ---------------------------
        #   b0 - Intercept
        # ---------------------------
        
        title = f"{bse_counter}.3) Regression equation"
        steps_mathjax.append(html_bg_level2(title=title))
    ...
    @cached_property
    def matrix_stderrors_latex(self) -> list[str]:
        """Standard errors"""
    ...
    @cached_property
    def normal_stderrors_latex(self) -> list[str]:
        """Standard errors"""
        ...
        title = (
            f"{bse_counter}.1) Standard error for the intercept "
            f"\\( (\\hat{{\\beta_{{0}}}}) \\)"
        )
        steps_mathjax.append(html_bg_level2(title=title))
        ...
        steps_mathjax.append(
            ...
            f"given by Equation \\( ({bse_counter}.1) \\) below."
        )
        ...
        steps_mathjax.append(f"\\( {self.table_normal_stderrors_latex} \\)")
        ...
        steps_mathjax.append(
            ...
            f"= \\frac{{\\sum\\limits_{{i=1}}^{{{self.bse_n}}} \\: (y_{{i}} "
            f"- \\hat{{y}}_{{i}})^{{2}}}}{{n - k}} "
            f"= \\frac{{{self.sse_rnd}}}{{{self.bse_n} - {self.bse_k}}} "
            f"= {self.mse_rnd} \\)"
        )
        ...
        steps_mathjax.append(
            f"Replace the above values into Equation \\({bse_counter}.1\\) and "
            f"work through as follows."
        )
        steps_mathjax.append(f"\\( {mse_formula} \\)")
        steps_mathjax.append(
            f"\\( \\quad = "
            f"\\sqrt{{{self.mse_rnd} \\: \\left(\\dfrac{{1}}{{{self.bse_n}}} "
            f"+ \\dfrac{{\\left({self.mean_x_rnd}"
            f"\\right)^{{2}}}}{{{self.sb_denom_rnd}}}\\right)}} \\)"
        )
        
        n_rnd = around(1 / self.bse_n, self.prm_decimals)
        term2 = self.mean_x ** 2 / self.sb_denom_rnd
        term2_rnd = around(term2, self.prm_decimals)
        ...
        title = (
            f"{bse_counter}.2) Standard error for the regression coefficient "
            f"\\( (\\hat{{\\beta_{{1}}}}) \\)"
        )
        steps_mathjax.append(html_bg_level2(title=title))
        ...
        steps_mathjax.append(
            ...
            f"\\( ({bse_counter}.2) \\) below"
        )
        
        steps_mathjax.append(
            f"\\( \\displaystyle s(\\hat{{\\beta_{{1}}}})"
            f"= \\sqrt{{\\dfrac{{MSE}}{{\\sum\\limits_{{i=1}}^{{{self.bse_n}}} "
            f"\\: x_{{i}}^{{2}} - n \\: \\bar{{x}}^{{2}}}}}} "
            f"\\qquad \\cdots \\qquad ({bse_counter}.2) \\)"
        )

        term = self.total_x_squared - self.bse_n * self.mean_x ** 2
        term_rnd = around(term, self.prm_decimals)
        steps_mathjax.append(
            f"\\( \\quad "
        ...
        steps_mathjax.append(
            f"\\( \\quad "
            f"= \\sqrt{{{around(self.mse / term, self.prm_decimals)}}} \\)"
        )
    ...
    def tstats_latex(self) -> list[str]:
        ...
        steps_mathjax.append(
            ...
            f"- \\: \\) Standard errors{self.html_str().stderrors}"
        )
        ...
        steps_mathjax.append(
            ...
            f"\\( s(\\hat{{\\beta}}) \\) into Equation \\( ({bse_counter}.1) \\) "
    ...
    def pvalues_latex(self) -> list[str]:
        ...
        self.bse_counter += 1
        bse_counter = self.bse_counter
        n, k, conf_level = self.bse_n, self.bse_k, self.conf_level
        ...
        for idx in range(len(pvalues)):
            title = (
                f"{bse_counter}.{idx + 1}) P value associated with "
                f"\\( \\beta_{{{idx}}} \\)"
            )
            steps_mathjax.append(html_bg_level2(title=title))
    ...
    @cached_property
    def conf_intervals_latex(self) -> list[str]:
        ...
        steps_mathjax.append(
            ...
            f"calculated using Equation \\( ({bse_counter}.1) \\) below."
        )
        
        eqtn_number = f"\\qquad \\cdots \\qquad ({bse_counter}.1)"
        bi = f"\\beta_{{i}}"
        eqtn_str = (
        ...
        n, k, sig_level = self.bse_n, self.bse_k, self.sig_level
        ...
        steps_mathjax.append(
            ...
            f"\\( ({bse_counter}.1) \\) and work through to get the "
        ...
        for idx in range(len(self.b)):
            ...
            se_times_t_rnd = around(se_times_t, self.prm_decimals)
    ...
    def fitted_and_residuals_latex(self) -> list[str]:
        ...
        ith = (
            f"(i = 1, 2, 3, 4, 5)" 
            if self.bse_k <= 6 else f"(i = 1, 2, 3, ..., {self.bse_k - 1})"
        )
        xi = f"x_{{1}}" if self.bse_k == 2 else f"x_{{i}} \\: for {ith}"
        
        vars_str = (
            "values of the independent variable" if self.bse_k == 2
            else "values of the respective independent variables"
        )
        
        calc_str = (
            "These calculations are presented below"
            if self.bse_k <= 5 else 
        ...
        title = f"{bse_counter}.1) Fitted values"
        steps_mathjax.append(html_bg_level2(title=title))
        ...
        for idx in range(self.bse_n):
            
            if max_show <= idx <= self.bse_n - 2:
                continue
            ...
            if idx == max_show - 1:
                ...
                steps_mathjax.append(
                    f"\\( \\qquad "
                    f"\\textit{{{self.bse_n - max_show} calculations omitted}} \\)"
                )
                steps_mathjax.append("\\( \\qquad\\qquad\\vdots \\)")
        ...
        title = f"{bse_counter}.2) Residuals"
        steps_mathjax.append(html_bg_level2(title=title))
