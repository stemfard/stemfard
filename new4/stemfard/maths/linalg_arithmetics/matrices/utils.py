...
# All valid operations (immutable tuple)
MATRIX_OPERATIONS: tuple[str, ...] = tuple(MAP_OPERATION.keys())

# ----------------------------
# Element-wise operations only
# ----------------------------
MATRIX_OPS_ELEMENTWISE: tuple[str, ...] = tuple(
    op for op in MATRIX_OPERATIONS if op != "matmul"
)

# ----------------------------
# Type hints (static)
# ----------------------------
MatrixOperations = Literal["add", "subtract", "multiply", "divide", "raise"]
ElementwiseMatrixOperations = Literal[
    "add", "subtract", "multiply", "divide", "raise", "mutmul"
]
...
def broadcast_and_expand_matrices(
    ...
    separately.
    """
    if operation not in MATRIX_OPS_ELEMENTWISE:
        raise OperationError(
            operation=operation,
            valid_operations=MATRIX_OPS_ELEMENTWISE
        )
...
def check_matrix_shapes(
    ...
    for op in operations:
        if op in MATRIX_OPS_ELEMENTWISE:
            if A.shape != B.shape:
                raise ShapeMismatchError(
...
def elementwise_apply(
    ...
    *,
    ...
    if broadcast:
        if operation is None or operation not in MATRIX_OPS_ELEMENTWISE:
