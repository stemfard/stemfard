...
from stemfard.core._io import final_result
...
class MatrixArithmetics(MatrixArithmeticsBackground):
    ...
    def _user_matrices(self, operation: str) -> list[str]:
    ...
    def linalg_add_latex(self) -> list[str]:
        ...
        steps_mathjax.extend(self._user_matrices(operation="add"))
        ...
        steps_mathjax.append(f"\\( \\quad {final_result(result_rnd)} \\)")
    ...
    def linalg_subtract_latex(self) -> list[str]:
        ...
        steps_mathjax.extend(self._user_matrices(operation="subtract"))
    ...
    def linalg_multiply_latex(self) -> list[str]:
        ...
        steps_mathjax.extend(self._user_matrices(operation="multiply"))
    ...
    def linalg_divide_latex(self) -> list[str]:
        ...
        steps_mathjax.extend(self._user_matrices(operation="divide"))
        ...
        for row in range(self.nrows):
            for col in range(self.nrows):
                ...
                if (
                    B_ij == -1 or B_ij == 0
                    or B_ij == 1 or "." in str_remove_tzeros(B_ij)
                ): # only take reciprical if it is rational
                    ...
                    if is_negative(B_ij):
                        B_ij_reciprocal_latex = (
                            str_color_math(f"\\left({B_ij_latex}\\right)")
                        )
        ...
        n = self.non_floats_in_b
        s = "" if n == 1 else "s"
        if decimals == -1 and n > 0:
            steps_mathjax.append(
                f"Take the reciprocal of the {n} applicable element{s} in "
                f"matrix \\( {self.param_b} \\) (i.e. non-decimal, non-zero "
                "and non-unity)."
            )
            steps_mathjax.append(
    ...
    def linalg_raise_latex(self) -> list[str]:
        ...
        steps_mathjax.extend(self._user_matrices(operation="raise"))
    ...
    def linalg_matmul_latex(self) -> list[str]:
        ...
        steps_mathjax.append(
            "The product \\( A \\cdot B \\) is evaluated as follows."
        )
        ...
        for row in range(nrows_a):
            for col in range(ncols_a):
                Ax[row, col] = str_color_math(A[row, col], color=StemConstants.HEXCOLORS[row])
        print(Ax)
        steps_mathjax(f"\\( {tex_to_latex(Ax)} \\)")
        for row in range(nrows_a):
            for col in range(ncols_b):
                ...
                # joined together
                _temp = ["".join(line) for line in row_index_step1]
                AB[row, col] = " + ".join(_temp)
                
                _temp = [" ".join(line) for line in row_index_step2]
                AB_mul[row, col] = " ".join(_temp)
