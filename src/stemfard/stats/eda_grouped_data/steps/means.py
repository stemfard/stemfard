...
from core.dframes import df_add_rows
...
from stats.eda_grouped_data.models import EDAGroupedDataParams
from stats.eda_grouped_data.syntax import _syntax_mean
from stats.eda_grouped_data.tables import table_grouped_mean, table_grouped_qtn


def _step_i_formula(fparams: EDAGroupedDataParams) -> list[str]:
    ...
    step_temp = html_style_bg(
        title="STEP I: Write down the formula for the mean"
    )
...
def _step_ii_table(fparams: EDAGroupedDataParams) -> list[str]:
    ...
    step_temp = html_style_bg(
        title="STEP II: Create a table for calculations"
    )
    steps_mathjax.append(step_temp)
    ...
    if fparams.is_calculate_assumed_mean:
        ...
        if fparams.formula == "x-a":
            ...
            class_width_str = (
                ...
                f"\\( \\textbf{{Step II}} \\)) appear in the third column "
...
def _step_iii_mean_of_t(fparams: EDAGroupedDataParams) -> list[str]:
    
    steps_mathjax = []
    
    step_temp = html_style_bg(
        title="STEP III: Calculate the mean of \\( t \\)"
    )
    steps_mathjax.append(step_temp)
    steps_mathjax.append(
...
def _step_iv_mean_of_x(fparams: EDAGroupedDataParams) -> list[str]:
    
    steps_mathjax = []
    
    step_temp = html_style_bg(
        title="STEP IV: Calculate the mean of \\( x \\)"
    )
    steps_mathjax.append(step_temp)
    ...
    if fparams.formula == "x-a":
        steps_mathjax.append(
            ...
            f"\\( t \\) calculated in \\( \\textbf{{STEP III}} \\) to the "
        ...
        steps_mathjax.append(
            ...
            f"mean of \\( t \\) calculated in \\( \\textbf{{STEP III}} \\) "
...
def _mean_assumed_mean_none(fparams: EDAGroupedDataParams) -> list[str]:
    ...
    steps_mathjax.append(
        ...
        f"These calculations are presented in \\( \\textbf{{Step II}} \\) "
        "below."
    )
    ...
    step_temp = html_style_bg(
        title="STEP III: Calculate the mean of \\( x \\)"
    )
    steps_mathjax.append(step_temp)
    steps_mathjax.append(
...
def _mean_given_assumed_mean(fparams: EDAGroupedDataParams) -> list[str]:
...
def sta_eda_grouped_mean_steps(fparams: EDAGroupedDataParams) -> ResultDict:
