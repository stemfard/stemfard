...
from stemfard.core.convert import dframe_to_mathjax_array
from stats.eda_grouped_data.tables import (
...
def percentiles_steps(
    ...
    step_temp = html_style_bg(
        title="STEP I: Create a table of cumulative frequencies"
    )
...
def _step_II_cumfreq_curve(
    ...
    dframe_latex = dframe_to_mathjax_array(
        df=dframe,
        outer_border=True,
        inner_hlines=True,
        inner_vlines=True
    ...
    steps_temp = html_style_bg(
        title=f"STEP II: Plot the cumulative frequency curve (ogive)"
    )
    ...
    steps_temp = html_style_bg(
        title=f"STEP III: Read the percentiles{s} from the graph"
    )
    steps_mathjax.append(steps_temp)
    ...
    if x_values:
        ...
        s = "" if n == 1 else "s"
        steps_temp = html_style_bg(
            title=f"STEP IV: Read the given \\( x \\) value{s} from the graph"
        )
        steps_mathjax.append(steps_temp)
        
        steps_mathjax.append(
            "Below is the cumulative curve that we had in "
            f"\\( \\textbf{{Step III}} \\). We have drawn it separately for "
...
def _step_II_formula(
    ...
    if len(percentiles) == 1:
        if percentiles[0] == 25:
            steps_temp = html_style_bg(
                title=(
                    "STEP II: Write the formula for the lower quartile "
                    f"(\\( Q_{{1}} \\))"
                )
            )
            steps_mathjax.append(steps_temp)
            ...
            steps_temp = html_style_bg(
                title=(
                    "STEP II: Write the formula for the median "
                    f"(\\( Q_{{2}} \\))"
                )
            )
            steps_mathjax.append(steps_temp)
            ...
            steps_temp = html_style_bg(
                title=(
                    f"STEP II: Write the formula for the upper quartile "
                    f"(\\( Q_{{3}} \\))"
                )
            )
            steps_mathjax.append(steps_temp)
            ...
            steps_temp = html_style_bg(
                title=f"STEP II: Write the formula for calculating percentiles"
            )
            steps_mathjax.append(steps_temp)
            steps_mathjax.append("The formula is given as follows,")
            steps_mathjax.append(
    ...
    steps_temp = html_style_bg(
        title="STEP II: Write the formula for calculating percentiles"
    )
    steps_mathjax.append(steps_temp)
    steps_mathjax.append("The formula is given as follows,")
    steps_mathjax.append(
...
def _calculations(
    ...
    steps_temp = html_style_bg(title=f"STEP III: Perform the calculations")
    steps_mathjax.append(steps_temp)
    steps_mathjax.append(
        "The calculations are performed using the formula given in "
        f"\\( \\textbf{{STEP II}} \\) as presented below."
    )
...
def sta_eda_grouped_percentiles_steps(
    data: ParamsData,
    percentile_params: ParamsPercentiles,
    plot_params: ParamsPlot
) -> dict[str, list[str]]:
...
def sta_eda_grouped_percentiles_steps() -> ResultDict:
    return None